...
public class RandomDbCreation {
	...
	private MarkDto getRandomMark() {
		...
		int mark = randomMarkValue();
		return new MarkDto(null, studentId , subjectId, mark);
	}

	private int randomMarkValue() {
	...
	private void createLecturers() {
		IntStream.rangeClosed(initialLecturerId, initialLecturerId + nLecturers).mapToObj(index -> 
			getRandomPerson(index)).forEach(lecturer -> {
			try {
	...
	private String randomPhone() {
		...
		int number = getRandomNumber(1000000, 9999999 + 1);
		return code  + "-" + number ;
	}

	private String randomBirthDate() {
		int year = getRandomNumber(1987, 2000);
		int month = getRandomNumber(1, 12 + 1);
		int day = getRandomNumber(1, 28 +1 );
